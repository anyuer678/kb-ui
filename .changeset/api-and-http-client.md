---
'@yuer678/kb-api': minor
'@yuer678/kb-utils': minor
---

新增可复用的后端模块 `@yuer678/kb-api` 与配套请求层：

**`@yuer678/kb-api`（新包）** —— 一个可直接 `import` 的 Express 5 + Zod 4 后端：
...
**`@yuer678/kb-utils` HTTP 请求层** —— 新增 `createHttp`：
