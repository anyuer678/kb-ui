...
| 🔧 进阶 | DatePicker（单日期/范围/多选）, Form（校验+动态字段）, Grid（响应式布局）, Select（搜索+键盘导航）, Table（排序/分页/固定列/行选择）, Tabs, Timeline |
| 📋 演示级 | Cascader（嵌套选择可用，缺异步加载）, Transfer（双列表迁移，缺搜索/分页）, Tree（基础树形，缺拖拽/虚拟滚动） |
...
| 测试 | Vitest（269 例单测）+ Playwright（e2e 19 项） |
| 文档 | Vitepress 1.6 |
| 版本管理 | changesets |
